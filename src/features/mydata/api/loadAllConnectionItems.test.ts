...
vi.mock('./httpClient', () => {
  const successData: Record<string, unknown> = {
    identity: { name: '김민준', birthYear: 1997, gender: 'MALE' },
    income: { annualGrossSalary: 48000000 },
    employment: { jobLabel: '직장인 (IT·기획)' },
...
describe('loadAllConnectionItems', () => {
  ...
  it('success 시나리오: 8개 항목 모두 success 상태가 되고 mock 데이터가 채워진다', async () => {
    await loadAllConnectionItems('success');

    const { items } = useConnectionStore.getState();

    expect(items.identity.status).toBe('success');
    expect(items.income.status).toBe('success');
    expect(items.employment.status).toBe('success');
    ...
    if (items.identity.status === 'success') {
      expect(items.identity.data.name).toBe('김민준');
    }
  });
